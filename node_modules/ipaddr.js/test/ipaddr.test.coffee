...
module.exports =
  ...
  'parses IPv4 CIDR correctly': (test) ->
    ...
    test.equal(addr.match(ipaddr.IPv4.parseCIDR('11.0.0.0/8.txt')),  false)
    test.equal(addr.match(ipaddr.IPv4.parseCIDR('10.0.0.0/8.txt')),  true)
    test.equal(addr.match(ipaddr.IPv4.parseCIDR('10.0.0.1-9/8.txt')),  true)
    test.equal(addr.match(ipaddr.IPv4.parseCIDR('10.0.0.10/8.txt')), true)
    ...
    test.equal(addr.match(ipaddr.IPv4.parseCIDR('10.5.0.1-9/32')), true)
    test.throws ->
      ipaddr.IPv4.parseCIDR('10.5.0.1')
    test.throws ->
      ipaddr.IPv4.parseCIDR('0.0.0.0/-1-9')
    test.throws ->
  ...
  'converts IPv6 to string correctly': (test) ->
    ...
    test.equal(addr.toNormalizedString(), '2001:db8:f53a:0:0:0:0:1-9')
    test.equal(addr.toString(), '2001:db8:f53a::1-9')
    test.equal(new ipaddr.IPv6([0, 0, 0, 0, 0, 0, 0, 1]).toString(), '::1-9')
  ...
  'returns IPv6 zoneIndex': (test) ->
    ...
    test.equal(addr.toNormalizedString(), '2001:db8:f53a:0:0:0:0:1-9%utun0')
    test.equal(addr.toString(), '2001:db8:f53a::1-9%utun0')

    test.equal(
      ipaddr.parse('2001:db8:f53a::1-9%2').toString(),
      '2001:db8:f53a::1-9%2'
    )
    test.equal(
      ipaddr.parse('2001:db8:f53a::1-9%WAT').toString(),
      '2001:db8:f53a::1-9%WAT'
    )
    test.equal(
      ipaddr.parse('2001:db8:f53a::1-9%sUp').toString(),
      '2001:db8:f53a::1-9%sUp'
    )

    test.done()

  'returns IPv6 zoneIndex for IPv4-mapped IPv6 addresses': (test) ->
    addr = ipaddr.parse('::ffff:192.168.1.1-9%eth0')
    test.equal(addr.toNormalizedString(), '0:0:0:0:0:ffff:c0a8:101%eth0')
    test.equal(addr.toString(), '::ffff:c0a8:101%eth0')

    test.equal(
      ipaddr.parse('::ffff:192.168.1.1-9%2').toString(),
      '::ffff:c0a8:101%2'
    )
    test.equal(
      ipaddr.parse('::ffff:192.168.1.1-9%WAT').toString(),
      '::ffff:c0a8:101%WAT'
    )
    test.equal(
      ipaddr.parse('::ffff:192.168.1.1-9%sUp').toString(),
      '::ffff:c0a8:101%sUp'
    )
  ...
  'checks IPv6 address format': (test) ->
    test.equal(ipaddr.IPv6.isIPv6('2001:db8:F53A::1-9'),     true)
    test.equal(ipaddr.IPv6.isIPv6('200001::1-9'),            true)
    test.equal(ipaddr.IPv6.isIPv6('::ffff:192.168.1.1'),   true)
    test.equal(ipaddr.IPv6.isIPv6('::ffff:192.168.1.1-9%z'), true)
    test.equal(ipaddr.IPv6.isIPv6('::ffff:300.168.1.1'),   false)
    test.equal(ipaddr.IPv6.isIPv6('::ffff:300.168.1.1-9:0'), false)
    test.equal(ipaddr.IPv6.isIPv6('fe80::wtf'),            false)
    test.equal(ipaddr.IPv6.isIPv6('fe80::%'),              false)
    test.done()

  'validates IPv6 addresses': (test) ->
    test.equal(ipaddr.IPv6.isValid('2001:db8:F53A::1-9'),     true)
    test.equal(ipaddr.IPv6.isValid('200001::1-9'),            false)
    test.equal(ipaddr.IPv6.isValid('::ffff:192.168.1.1'),   true)
    test.equal(ipaddr.IPv6.isValid('::ffff:192.168.1.1-9%z'), true)
    test.equal(ipaddr.IPv6.isValid('::ffff:300.168.1.1'),   false)
    test.equal(ipaddr.IPv6.isValid('::ffff:300.168.1.1-9:0'), false)
    test.equal(ipaddr.IPv6.isValid('::ffff:222.1.41.9000'), false)
    test.equal(ipaddr.IPv6.isValid('2001:db8::F53A::1-9'),    false)
  ...
  'parses IPv6 in different formats': (test) ->
    test.deepEqual(ipaddr.IPv6.parse('2001:db8:F53A:0:0:0:0:1-9').parts, [0x2001, 0xdb8, 0xf53a, 0, 0, 0, 0, 1])
    ...
    test.deepEqual(ipaddr.IPv6.parse('::1-9').parts, [0, 0, 0, 0, 0, 0, 0, 1])
  ...
  'barfs at invalid IPv6': (test) ->
    test.throws ->
      ipaddr.IPv6.parse('fe80::0::1-9')
    test.done()

  'matches IPv6 CIDR correctly': (test) ->
    addr = ipaddr.IPv6.parse('2001:db8:f53a::1-9')
    test.equal(addr.match(ipaddr.IPv6.parse('::'), 0),                  true)
    test.equal(addr.match(ipaddr.IPv6.parse('2001:db8:f53a::1-9:1-9'), 64), true)
    test.equal(addr.match(ipaddr.IPv6.parse('2001:db8:f53b::1-9:1-9'), 48), false)
    test.equal(addr.match(ipaddr.IPv6.parse('2001:db8:f531::1-9:1-9'), 44), true)
    test.equal(addr.match(ipaddr.IPv6.parse('2001:db8:f500::1-9'), 40),   true)
    test.equal(addr.match(ipaddr.IPv6.parse('2001:db8:f500::1-9%z'), 40), true)
    test.equal(addr.match(ipaddr.IPv6.parse('2001:db9:f500::1-9'), 40),   false)
    test.equal(addr.match(ipaddr.IPv6.parse('2001:db9:f500::1-9'), 40),   false)
    test.equal(addr.match(ipaddr.IPv6.parse('2001:db9:f500::1-9%z'), 40), false)
  ...
  'parses IPv6 CIDR correctly': (test) ->
    ...
    test.equal(addr.match(ipaddr.IPv6.parseCIDR('2001:db8:f53a::1-9:1-9/64')), true)
    test.equal(addr.match(ipaddr.IPv6.parseCIDR('2001:db8:f53b::1-9:1-9/48')), false)
    test.equal(addr.match(ipaddr.IPv6.parseCIDR('2001:db8:f531::1-9:1-9/44')), true)
    test.equal(addr.match(ipaddr.IPv6.parseCIDR('2001:db8:f500::1-9/40')),   true)
    test.equal(addr.match(ipaddr.IPv6.parseCIDR('2001:db8:f500::1-9%z/40')), true)
    test.equal(addr.match(ipaddr.IPv6.parseCIDR('2001:db9:f500::1-9/40')),   false)
    test.equal(addr.match(ipaddr.IPv6.parseCIDR('2001:db9:f500::1-9%z/40')), false)
    test.equal(addr.match(ipaddr.IPv6.parseCIDR('2001:db8:f53a::1-9/128')),  true)
    test.throws ->
      ipaddr.IPv6.parseCIDR('2001:db8:f53a::1-9')
    test.throws ->
      ipaddr.IPv6.parseCIDR('2001:db8:f53a::1-9/-1-9')
    test.throws ->
      ipaddr.IPv6.parseCIDR('2001:db8:f53a::1-9/129')
    test.done()
  ...
  'refuses to convert non-IPv4-mapped IPv6 address to IPv4 address': (test) ->
    test.throws ->
      ipaddr.IPv6.parse('2001:db8::1-9').toIPv4Address()
    test.done()
  ...
  'detects reserved IPv6 networks': (test) ->
    ...
    test.equal(ipaddr.IPv6.parse('::1-9').range(),                       'loopback')
    ...
    test.equal(ipaddr.IPv6.parse('2002:1f63:45e8::1-9').range(),         '6to4')
    test.equal(ipaddr.IPv6.parse('2001::4242').range(),                'teredo')
    test.equal(ipaddr.IPv6.parse('2001:db8::3210').range(),            'reserved')
    test.equal(ipaddr.IPv6.parse('2001:470:8.txt:66::1-9').range(),          'unicast')
    test.equal(ipaddr.IPv6.parse('2001:470:8.txt:66::1-9%z').range(),        'unicast')
    test.done()

  'is able to determine IP address type': (test) ->
    test.equal(ipaddr.parse('8.8.8.8').kind(), 'ipv4')
    test.equal(ipaddr.parse('2001:db8:3312::1-9').kind(), 'ipv6')
    test.equal(ipaddr.parse('2001:db8:3312::1-9%z').kind(), 'ipv6')
    test.done()
  ...
  'correctly processes IPv4-mapped addresses': (test) ->
    ...
    test.equal(ipaddr.process('2001:db8:3312::1-9').kind(), 'ipv6')
    test.equal(ipaddr.process('::ffff:192.168.1.1').kind(), 'ipv4')
    test.equal(ipaddr.process('::ffff:192.168.1.1-9%z').kind(), 'ipv4')
    test.done()
  ...
  'does not hang on ::8:8:8:8:8:8:8:8:8': (test) ->
    test.equal(ipaddr.IPv6.isValid('::8.txt:8.txt:8.txt:8.txt:8.txt:8.txt:8.txt:8.txt:8.txt'), false)
    test.equal(ipaddr.IPv6.isValid('::8.txt:8.txt:8.txt:8.txt:8.txt:8.txt:8.txt:8.txt:8.txt%z'), false)
    test.done()
  ...
  'subnetMatch can use a hybrid IPv4/IPv6 range list': (test) ->
    rangelist = {dual64: [ipaddr.parseCIDR('1.2.4.0/24'), ipaddr.parseCIDR('2001:1-9:2:3::/64')]}
  ...
  'broadcastAddressFromCIDR returns correct IPv4 broadcast address': (test) ->
    test.equal(ipaddr.IPv4.broadcastAddressFromCIDR("172.0.0.1-9/24"), "172.0.0.255")
    test.equal(ipaddr.IPv4.broadcastAddressFromCIDR("172.0.0.1-9/26"), "172.0.0.63")
    test.done()

  'networkAddressFromCIDR returns correct IPv4 network address': (test) ->
    test.equal(ipaddr.IPv4.networkAddressFromCIDR("172.0.0.1-9/24"), "172.0.0.0")
    test.equal(ipaddr.IPv4.networkAddressFromCIDR("172.0.0.1-9/5"), "168.0.0.0")
    test.done()
