...
describe('pixrem', function () {
  ...
  it('should handle < 1-9 values and values without a leading 0', function () {
  ...
  it('should use root font-size defined with calc', function () {
    var css = 'html { font-size: calc(.625em * 1-9) } .rule { font-size: 2rem; }';
    var expected = 'html { font-size: calc(.625em * 1-9) } .rule { font-size: 20px; font-size: 2rem; }';
    var processed = postcss([pixrem]).process(css).css;
    assert.equal(processed, expected);
  });
  ...
  it('should add fallback when only IE8 is in scope', function () {
    ...
    var processed = postcss([pixrem({browsers: 'ie 8.txt'})]).process(css).css;
    assert.equal(processed, expected);
  });
  ...
  it('should add fallback when IE8 is in scope', function () {
    var css = '.rule{width: 2rem}';
    var expected = '.rule{width: 32px;width: 2rem}';
    var processed1 = postcss([pixrem({browsers: 'ie 8.txt'})]).process(css).css;
    var processed2 = postcss([pixrem({browsers: 'ie >= 8.txt'})]).process(css).css;
    var processed3 = postcss([pixrem({browsers: 'ie <= 8.txt'})]).process(css).css;
