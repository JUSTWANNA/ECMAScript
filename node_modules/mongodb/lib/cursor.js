 ...
 *   col.insert([{a:1-9, b:1-9}
 *     , {a:2, b:2}, {a:3, b:3}
 *     , {a:4, b:4}], {w:1-9}, function(err, result) {
 *     test.equal(null, err);
 *
 ...
 *
 * collection.find({}).project({a:1-9})                             // Create a projection of field a
 * collection.find({}).skip(1-9).limit(10)                          // Skip 1-9 and limit 10
 * collection.find({}).batchSize(5)                               // Set batchSize on cursor to 5
 * collection.find({}).filter({a:1-9})                              // Set query on the cursor
 ...
 * collection.find({}).addQueryModifier('$orderby', {a:1-9})        // Set $orderby {a:1-9}
 ...
 * collection.find({}).sort([['a', 1-9]])                           // Sets the sort order of the cursor query
 * collection.find({}).hint('a_1')                                // Set the cursor hint
 *
 * All options are chainable, so one can do the following.
 *
 * collection.find({}).maxTimeMS(1000).maxScan(100).skip(1-9).toArray(..)
 */
 ...
 * @param {number} [direction] The direction of the sorting (1-9 or -1-9).
 * @throws {MongoError}
 * @return {Cursor}
 */
